...
Create an environment with PyTorch (matching your CUDA driver).

Install Detectron2 (from source is common):
...
Resize images & masks to a fixed resolution (e.g., 512×512).
...
Split into train/test COCO JSON.

Register datasets in Detectron2.

Train Mask R-CNN using:

COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml

single class: water

Evaluate using COCO metrics (AP / IoU, etc.)

(Optional) Save example predicted masks, compute basic per-image metrics, export CSV summaries.
